...
<head>
  ...
  <script src="js/navebarb.js"></script>
...
function showreason(reason) {
   console.log(reason);
   document.getElementById('reason').placeholder = reason;
}
 /*顯示人名*/
...
function itemsubmit() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      console.log(data);
      window.location.assign(window.location.href); //重整
      //reasonElement.value=null; //啟動原因成功到資料庫後，把啟動原因去掉
    },
    error:function(XMLHttpRequest,status,error){
        console.log(error)   
    }
...
<div class="ui-block-a"  style="height:80px !important">
<div class="ui-bar ui-bar-a" >
<div class="people-remark1">啟動原因</div>
</div>
</div>
...
<div class="ui-bar ui-bar-a">
  <table>
        <tr>
            ...
            <td width="1%">
                 <button class="ui-btn ui-btn-inline ui-corner-all" id="btn-reason" onclick="itemsubmit()">送出</button>
            </td>
        </tr>
  </table>
</div>
...
<div class="ui-bar ui-bar-a">
    ...
    <table>
        <tr>
            ...
            <td width="87%">
                <input type="text" id="h" name="people"/>
            </td>
        </tr>
    </table>
<HR>
    ...
    <table>
        <tr>
            <td width="13%">
             ...
             <td width="87%">
                <input type="text" id="i" name="people"/>
            </td>
        </tr>
    </table>
...
<%   
   ...
   name = beanDB.show_one_people("h");
       ...
       <%  }else{
            String s ="";
            s="請輸入人名，提醒： (上班時段)急診資深醫師(非上班時段)急診資深醫師";
...
<%   
   name = beanDB.show_one_people("i");
       ...
       <%  }else{
            String s ="";
            s="請輸入人名，提醒： (上班時段)急診護理長(非上班時段)急診小組長";
       %>
            <script>
                showpeople("i","<%=s%>");
            </script> 
...
<!--載啟動原因-->
<%   
   String reason = "請輸入啟動原因";
   if(beanDB.showreason() == null || beanDB.showreason() == "")
     reason = "請輸入啟動原因";
   else
     reason = beanDB.showreason();
%>
    <script>
        showreason("<%=reason%>");
    </script>
